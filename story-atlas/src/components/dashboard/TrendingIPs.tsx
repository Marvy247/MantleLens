...
interface TrendingIPsProps {
  mostRemixedIPs: IPItem[];
}

export default function TrendingIPs({ mostRemixedIPs }: TrendingIPsProps) {
    ...
    <Card className="bg-zinc-900 border-zinc-800 p-6">
      ...
      {mostRemixedIPs.length > 0 ? (
        <div className="space-y-3">
          {mostRemixedIPs.slice(0, 10).map((ip, index) => (
            <div
              key={ip.ipId}
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <div className="text-right">
                  <p className="text-lg font-bold text-white">{ip.count}</p>
                  <p className="text-xs text-zinc-500">derivatives</p>
                </div>
              </div>
            </div>
          ))}
